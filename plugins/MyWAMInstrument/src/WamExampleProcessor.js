...
const getWamExampleProcessor = (moduleId) => {
	...
	const zeroInput = [new Float32Array(128), new Float32Array(128)];
	...
	class WamExampleProcessor extends WamProcessor {
		...
		_process(startSample, endSample, inputs, outputs) {
			let input = inputs[0].length === 0 ? zeroInput : inputs[0];
	...
	try {
		registerProcessor('WebAudioModuleMyWAMInstrument', WamExampleProcessor);
	} catch (error) {
		console.warn(error);
	}
